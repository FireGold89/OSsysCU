...
def _styles():
    return {
        'title': ParagraphStyle(
            'title', fontName=FONT, fontSize=18, leading=22, alignment=TA_CENTER,
            textColor=colors.HexColor('#1e3a5f'), spaceAfter=4,
        ),
        'subtitle': ParagraphStyle(
            'subtitle', fontName=FONT, fontSize=11, leading=14, alignment=TA_CENTER,
            textColor=colors.HexColor('#64748b'), spaceAfter=10,
        ),
        'h2': ParagraphStyle(
            'h2', fontName=FONT, fontSize=12, leading=15, textColor=colors.HexColor('#1e3a5f'),
            spaceBefore=8, spaceAfter=6,
        ),
        'body': ParagraphStyle(
            'body', fontName=FONT, fontSize=9, leading=12, textColor=colors.HexColor('#334155'),
        ),
        'small': ParagraphStyle(
            'small', fontName=FONT, fontSize=8, leading=10, textColor=colors.HexColor('#64748b'),
        ),
        'bullet': ParagraphStyle(
            'bullet', fontName=FONT, fontSize=9, leading=13, leftIndent=10,
            textColor=colors.HexColor('#334155'), spaceAfter=2,
        ),
        'cell': ParagraphStyle(
            'cell', fontName=FONT, fontSize=8, leading=10, textColor=colors.HexColor('#334155'),
        ),
        'cell_r': ParagraphStyle(
            'cell_r', fontName=FONT, fontSize=8, leading=10, alignment=TA_RIGHT,
            textColor=colors.HexColor('#334155'),
        ),
        'cell_b': ParagraphStyle(
            'cell_b', fontName=FONT, fontSize=8, leading=10, textColor=colors.HexColor('#0f172a'),
        ),
    }


def _header_footer(canvas, doc, company_name: str, project_code: str):
    canvas.saveState()
...
def generate_boss_qs_report(summary: dict, sc_list: list | None = None,
    ...
    meta_rows = [
        ...
        ['項目名稱（英）', Paragraph(_plain(en or '—', 80), styles['cell']), '客戶', _plain(project.get('client'))],
        ['項目名稱（中）', Paragraph(_plain(zh or '—', 80), styles['cell']), '主承建商', _plain(project.get('main_contractor'), 40)],
    ...
    cat_tbl = Table(_category_rows(sc_list), colWidths=[35 * mm, 45 * mm, 45 * mm, 43 * mm])
    ...
    if sc_list:
        ...
        for sc in sorted(sc_list, key=lambda x: (x.get('sc_no') or '')):
            ...
            sc_rows.append([
                ...
                Paragraph(_plain(sc.get('company_name_en') or sc.get('company_name_zh'), 28), styles['cell']),
                _money(ca),
                _money(paid),
                _money(rem),
                _pct(pct),
    ...
    doc.build(
        story,
        onFirstPage=lambda c, d: _header_footer(c, d, company, code),
        onLaterPages=lambda c, d: _header_footer(c, d, company, code),
    )
